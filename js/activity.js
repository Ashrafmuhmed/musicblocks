...
import { supportedLanguages , DEFAULT_LANGUAGE } from "./constants";
/*
   globals
...
class Activity {
    ...
    /**
     * Sets the language of the activity based on user preference or browser default.
     * @async
     * @throws {Error} If the language change fails.
     * @returns {void}
     */
    async setLanguage() {
        try {
            ...
            if (this.storage.languagePreference !== undefined) {
                lang = this.storage.languagePreference; // Use user's preferred language
                console.log("Setting language to user preference:", lang);
            } else {
                lang = navigator.language; // Use browser's language
                if (lang.includes("-")) {
                    lang = lang.slice(0, lang.indexOf("-"));
                }
                console.log("Setting language to browser default:", lang);
            }

            if (!supportedLanguages.includes(lang)) {
                lang = DEFAULT_LANGUAGE; // Fallback to default language
                console.warn(`Unsupported language: ${lang}. Falling back to 'en'.`);
            }

            // Change language using i18next with a callback
